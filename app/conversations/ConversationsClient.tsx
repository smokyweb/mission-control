      ...
      <div className="w-80 shrink-0 border-r border-[#2A2A3E] bg-[#0A0A0F] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {filtered.map((session) => {
            ...
            const isSelected = selected?.sessionId === session.sessionId && selected?.isArchive === session.isArchive;
            return (
              <button
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm font-medium text-white truncate">
                        {session.channel}
                        {session.isArchive && (
                          <span className="ml-1 text-xs text-yellow-600">[archived]</span>
                        )}
                      </span>
                    ...
                    <p className="text-xs text-gray-500 truncate mt-0.5">
                      {lastMsg?.text.slice(0, 80) ?? "No messages"}
                    </p>
          ...
          <div className="px-6 py-4 border-b border-[#2A2A3E] bg-[#0A0A0F] flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold text-white">
                {selected.channel}
                {selected.isArchive && (
                  <span className="ml-2 text-xs text-yellow-600 font-normal">[archived session]</span>
                )}
              </h2>
              <p className="text-xs text-gray-500" suppressHydrationWarning>
                Started {formatTime(selected.startedAt)} · {selected.messageCount} messages
              </p>
            </div>
          </div>
