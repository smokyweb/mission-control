...
interface Task {
  ...
  difficulty?: "gemini" | "deepseek" | "chat" | "chatpro" | "sonnet" | "codex";
  assignedTo?: "kevin" | "brenthomer";
  assignedModel?: "google/gemini-2.5-flash" | "deepseek/deepseek-chat" | "openai/gpt-5.4" | "openai/gpt-5.4-pro" | "anthropic/claude-sonnet-4-6" | "openai/gpt-4o" | "codex";
...
function difficultyToModel(difficulty: "gemini" | "deepseek" | "chat" | "chatpro" | "sonnet" | "codex" | null): string | undefined {
  switch (difficulty) {
    case "gemini":
    ...
    case "codex":
      return "anthropic/claude-sonnet-4-6"; // Sonnet directs Codex
    default:
      return undefined;
  }
}
  ...
  return (
    <div className="flex items-center gap-1.5 bg-[#0A0A0F] border border-[#2A2A3E] rounded-lg px-2 py-1 group/chip">
      ...
      <a href={url} target="_blank" rel="noreferrer"
        className="text-sm text-gray-300 hover:text-white truncate max-w-[120px]" title={att.name}>
        {att.name}
      </a>
      <span className="text-sm text-gray-600 shrink-0">{formatBytes(att.size)}</span>
      <button onClick={onDelete}
        className="text-gray-600 hover:text-red-400 opacity-0 group-hover/chip:opacity-100 transition-opacity text-sm ml-0.5 leading-none">
        ?
      </button>
    </div>
  );
  ...
  return (
    <div>
      ...
      >
        ...
        <p className="text-sm text-gray-400">Drop files here or <span className="text-blue-400 underline">browse</span></p>
        <p className="text-sm text-gray-600 mt-0.5">Images, PDFs, docs, spreadsheets�</p>
      </div>
      <input
        ref={inputRef}
        type="file"
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {files.map((f, i) => (
            <div key={i} className="flex items-center gap-1.5 bg-[#0A0A0F] border border-[#2A2A3E] rounded-lg px-2 py-1">
              ...
              <span className="text-sm text-gray-300 truncate max-w-[120px]">{f.name}</span>
              <span className="text-sm text-gray-600 shrink-0">{formatBytes(f.size)}</span>
              <button onClick={() => onFiles(files.filter((_, j) => j !== i))}
                className="text-gray-600 hover:text-red-400 text-sm ml-0.5 leading-none">?</button>
            </div>
          ))}
        </div>
      )}
...
export default function TasksClient() {
  ...
  const [newDifficulty, setNewDifficulty] = useState<"gemini" | "deepseek" | "chat" | "chatpro" | "sonnet" | "codex">("chat");
  ...
  const addTask = async () => {
    ...
    try {
      ...
      const isCodex = newDifficulty === "codex";
      const assignedModel = newAssignedTo === "brenthomer" ? "openai/gpt-4o" : difficultyToModel(newDifficulty);
      // For Codex tasks: route to axeldev (Sonnet) who will direct Codex
      const assignedAgent = isCodex ? "axeldev" : undefined;
      // Prepend Codex instruction to description
      const codexPrefix = isCodex
        ? `[CODEX TASK] Use the codex skill to build this with Codex CLI. Sonnet (you) should analyze the requirements, write a detailed prompt, then spawn Codex via \`codex exec --dangerously-bypass-approvals-and-sandbox\` to implement it.\n\n`
        : "";
      const finalDesc = codexPrefix + (newDesc.trim() || "");
      let res: Response;
      if (stagedFiles.length > 0) {
        ...
        if (assignedAgent) form.append("assignedAgent", assignedAgent);
  ...
  const filtered = tasks.filter(t => {
    ...
    const matchesSearch = 
      ...
      t.assignedModel?.toLowerCase().includes(q) ||
    ...
    <div>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          placeholder="Search tasks by title, description, agent..."
          className="w-full bg-[#1A1A2E] border border-[#2A2A3E] rounded-xl pl-10 pr-10 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[#3A3A4E]"
        />
        {searchQuery && (
      ...
      {searchQuery && (
        <p className="text-xs text-gray-500 mb-3">{filtered.length} result{filtered.length !== 1 ? 's' : ''} for &ldquo;{searchQuery}&rdquo;</p>
      )}
      {/* Filter + Add */}
      ...
      {showAdd && (
        <div className="bg-[#1A1A2E] border border-blue-500/30 rounded-xl p-5 mb-6 animate-in fade-in duration-200">
          ...
          <div className="mb-3">
            <label className="block text-sm text-gray-400 mb-2 font-medium">Agent (auto-assigns model)</label>
        ...
        { value: "sonnet"   as const, label: "?? Sonnet",   hint: "claude-sonnet-4-6",  color: "bg-orange-900/30 border-orange-700/50 text-orange-300" },
                { value: "codex"    as const, label: "⚡ Codex",    hint: "Sonnet directs Codex", color: "bg-yellow-900/30 border-yellow-700/50 text-yellow-300" },
              ].map(option => (
                <label key={option.value} className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className={`px-2 py-1 rounded text-sm font-medium border ${option.color} ${newDifficulty === option.value ? 'ring-1 ring-white/30' : ''}`}>
                    {option.label}
                  </span>
                  {newDifficulty === option.value && <span className="text-sm text-gray-500">({option.hint})</span>}
                </label>
              ))}
            </div>
          </div>
          ...
          {/* Assign To */}
          <div className="mb-3">
            <label className="block text-sm text-gray-400 mb-2 font-medium">Assign to</label>
            <div className="flex gap-3">
              {[
                ...
                <label key={option.value} className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 cursor-pointer"
                  />
                  <span className={`px-2 py-1 rounded text-sm font-medium border ${option.color} ${newAssignedTo === option.value ? 'ring-1 ring-white/30' : ''}`}>
                    {option.label}
                  </span>
                  {newAssignedTo === option.value && <span className="text-sm text-gray-500">({option.hint})</span>}
                </label>
              ))}
            </div>
          </div>
      ...
      {reviewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4"
          ...
          <div className="bg-[#1A1A2E] border border-purple-500/40 rounded-2xl p-6 w-full max-w-lg shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-gray-400 mb-4 truncate">Task: <span className="text-purple-300">{reviewModal.title}</span></p>

            <div className="mb-4">
              <label className="block text-sm text-gray-400 mb-1.5 font-medium">?? What did you do? <span className="text-gray-600">(summary)</span></label>
              <textarea
                value={reviewSummary}
            ...
            <div className="mb-5">
              <label className="block text-sm text-gray-400 mb-1.5 font-medium">? Questions for Kevin <span className="text-gray-600">(optional)</span></label>
              <textarea
      ...
      {returnModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4"
          ...
          <div className="bg-[#1A1A2E] border border-blue-500/40 rounded-2xl p-6 w-full max-w-lg shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-gray-400 mb-4 truncate">Task: <span className="text-blue-300">{returnModal.title}</span></p>

            <div className="mb-5">
              <label className="block text-sm text-gray-400 mb-1.5 font-medium">What needs to be fixed?</label>
              <textarea
      ...
      {approveModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4"
          ...
          <div className="bg-[#1A1A2E] border border-green-500/40 rounded-2xl p-6 w-full max-w-lg shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-gray-400 mb-4 truncate">Task: <span className="text-green-300">{approveModal.title}</span></p>

            <div className="mb-5">
              <label className="block text-sm text-gray-400 mb-1.5 font-medium">Add a note <span className="text-gray-600">(optional)</span></label>
              <textarea
                value={approveNote}
              ...
              <div key={task.id}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-xl font-medium ${task.status === "completed" ? "line-through text-gray-500" : "text-white"}`}>
                      <Link href={`/tasks/${task.id}`} className="text-inherit no-underline hover:underline hover:text-yellow-300 transition-colors cursor-pointer">
                        {task.title}
                      </Link>
                    </p>
                    {task.description && (
                    ...
                    <div className="flex items-center gap-3 mt-2 flex-wrap">
                      ...
                      {task.assignedTo === "brenthomer" && (
                        <span className="text-sm px-2 py-0.5 rounded-full border bg-amber-900/30 border-amber-700/50 text-amber-300">Brent</span>
                      )}
                      <span className="text-sm text-gray-600" suppressHydrationWarning>{formatDate(task.createdAt)}</span>
                      {task.completedAt && (
                        <span className="text-sm text-gray-600" suppressHydrationWarning>Done {formatDate(task.completedAt)}</span>
                      )}
                      {task.reviewedAt && (
                        <span className="text-sm text-gray-600" suppressHydrationWarning>Sent for review {formatDate(task.reviewedAt)}</span>
                      )}
                      {task.comments && task.comments.length > 0 && (
                        <span className="text-sm px-2 py-0.5 rounded-full bg-gray-800/50 text-gray-300 border border-gray-700/50">
                      ...
                      <div className="mt-3 space-y-2">
                        {task.reviewSummary && (
                          <div className="bg-purple-900/20 border border-purple-800/40 rounded-lg px-3 py-2">
                            <p className="text-sm font-semibold text-purple-300 mb-1">?? What was done</p>
                        ...
                        {task.reviewQuestions && (
                          <div className="bg-yellow-900/20 border border-yellow-800/40 rounded-lg px-3 py-2">
                            <p className="text-sm font-semibold text-yellow-300 mb-1">? Questions for you</p>
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-2">
                                  <span className={`text-sm font-medium ${
                                    ...
                                    {c.author === "kevin" ? "Kevin" : c.author}
                                  </span>
                                  <span className="text-sm text-gray-600" suppressHydrationWarning>
                                    {formatRelativeTime(c.createdAt)}
                                  </span>
                                </div>
                                <p className="text-sm text-gray-300 mt-0.5 whitespace-pre-wrap">{c.text}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    {/* Add Comment */}
                    <div className={`mt-3 ${!task.comments?.length ? "border-t border-[#2A2A3E] pt-3" : ""}`}>
                      <textarea
                        ref={el => { commentRefs.current[task.id] = el; }}
                        ...
                        className="w-full bg-[#0A0A0F] border border-[#2A2A3E] rounded-lg px-2.5 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50 resize-none overflow-hidden min-h-[96px]"
                      ...
                      <div className="flex justify-end mt-1.5">
                        <button
                          ...
                          onClick={() => {
                            addComment(task.id);
                            setTimeout(() => autoResize(commentRefs.current[task.id]), 50);
                          }}
                          disabled={!commentTexts[task.id]?.trim() || commentSaving[task.id]}
                          className="px-3 py-1.5 text-sm rounded-lg bg-[#2A2A3E] text-gray-300 hover:text-white hover:bg-[#3A3A4E] disabled:opacity-40 transition-colors"
                        >
                          {commentSaving[task.id] ? "�" : "Send"}
                        </button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-1 shrink-0">
                    ...
                    {task.status === "open" && (
                      <button onClick={() => updateStatus(task.id, "in-progress")} title="Start"
                        className="px-2 py-1 text-sm rounded bg-blue-900/40 text-blue-300 hover:bg-blue-900/70 transition-colors">?</button>
                    )}
                    {/* In-progress: send to review or complete */}
                    {task.status === "in-progress" && (
                      <button onClick={() => openReviewModal(task)} title="Send for Review"
                        className="px-2 py-1 text-sm rounded bg-purple-900/40 text-purple-300 hover:bg-purple-900/70 transition-colors">?? Review</button>
                    )}
                    {(task.status === "open" || task.status === "in-progress") && (
                      <button onClick={() => updateStatus(task.id, "completed")} title="Complete"
                        className="px-2 py-1 text-sm rounded bg-green-900/40 text-green-300 hover:bg-green-900/70 transition-colors">?</button>
                    )}
                    {/* Review: approve or return */}
                    {task.status === "review" && (
                      <>
                        <button onClick={() => setApproveModal({ taskId: task.id, title: task.title })} title="Approve & Complete"
                          className="px-2 py-1 text-sm rounded bg-green-900/40 text-green-300 hover:bg-green-900/70 transition-colors">? Approve</button>
                        <button onClick={() => setReturnModal({ taskId: task.id, title: task.title })} title="Return to In Progress"
                          className="px-2 py-1 text-sm rounded bg-blue-900/40 text-blue-300 hover:bg-blue-900/70 transition-colors">? Return</button>
                      </>
                    )}
                    {/* Completed: reopen */}
                    {task.status === "completed" && (
                      <button onClick={() => updateStatus(task.id, "open")} title="Reopen"
                        className="px-2 py-1 text-sm rounded bg-yellow-900/40 text-yellow-300 hover:bg-yellow-900/70 transition-colors">?</button>
                    )}
                    <button onClick={() => deleteTask(task.id)} title="Delete"
                      className="px-2 py-1 text-sm rounded bg-red-900/40 text-red-400 hover:bg-red-900/70 transition-colors">?</button>
                  </div>
                </div>
              </div>
            );
